...
@section('title', 'Daftar Stok Produk Bahan')
        ...
        <div class="table-responsive text-nowrap">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-6">
                        <h4 class="card-title
                            d-flex align-items-center">Daftar Stok Produk Bahan
                        </h4>
                    </div>
                    @if ($blade_user->hasPermissionTo('can_create_admin_product_ingredient_stocks'))
                        <div class="col-md-6 text-end">
                            <a href="{{ route('admin.products.ingredient-stocks.create') }}" class="btn btn-default"><i
                                    class="bx bx-plus"></i> &nbsp;Tambah Stok</a>
                        </div>
                    @endif
                </div>
            ...
            <table class="table table-hover">
                <thead>
                    ...
                    <tr>
                        ...
                        <th scope="col">Ditambahkan / Tersedia</th>
                ...
                <tbody class="table-border-bottom-0">
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td>
                                <a
                                    href="{{ $purchase ? route('admin.purchases.show', $purchase->id) : 'javascript::void(0)' }}">{{ $purchase ? $purchase->invoice_number : '-' }}</a>
                            </td>
                            <td>
                                {{ $item->product_name }}
                                @if ($item->code)
                            ...
                            <td>{{ $item->amount_added . ' / ' . $item->amount_available }} @if ($item->amount_available == 0)
                                ...
                                @if ($item->expired_at && Carbon\Carbon::parse($item->expired_at)->isPast() && $item->amount_available > 0)
                                    <br><small><small><small><span
                                                    class="badge bg-danger">Kadaluwarsa</span></small></small></small>
                                @endif
                            </td>
                            ...
                            <td>
                                <ul class="list-inline hstack gap-2 mb-0">
                                    @include('layouts.admin.partials.act_delete', [
                                        'title' => 'Stok Produk Bahan',
                                        'href' => route('admin.products.ingredient-stocks.destroy', $item->id),
